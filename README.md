...
**Engineering rigor:** 260 automated tests (unit + live-network + live-LLM tiers), green on every push via GitHub Actions.
...
tests/    260 tests — 252 unit (always run) + 8 live-network/live-LLM (opt-in via -m)
```
...
```bash
pytest -v                 # 252 unit tests, no network/LLM required
...
**Since then: the reconciler's specificity has been hardened and independently generalization-tested.** Running the real Coordinator against 8 large tech companies confirmed to carry no restatement fingerprint first surfaced an apparent 25% false-positive rate — investigated rather than reported at face value, it resolved into **eleven separate, confirmed root causes** (period-resolution bugs, an XBRL instant-fact edge case, an MD&A chunking bug, a DSPy-compiled artifact silently freezing stale instructions, and more), each diagnosed against live data and fixed one at a time, landing the reconciler's own false-positive rate at **0/33 (0.0%)** given correct inputs — see [`docs/specificity-check-results.md`](docs/specificity-check-results.md). That 0% was measured only against the companies its own fixes were diagnosed against, so it was tested for overfitting directly: a second, disjoint 8-company set untouched by any fix found a real 45.5% false-positive rate, traced to a segment/acquisition sub-figure mislabeled under its bare metric name (**fixed**, confirmed eliminated on re-run) and a GAAP/non-GAAP ambiguity in "operating income" prose that an exact-match dictionary structurally can't disambiguate (**left open, documented**, not guessed at) — re-measured at **33.3% (3/9)**, with every remaining miss traced to that one known, open cause. Full write-up: [`docs/fresh-holdout-specificity-results.md`](docs/fresh-holdout-specificity-results.md). Every requirement, phase, and hardening pass in one place: 📒 [The Full Ledger (PDF)](docs/field-guide.pdf).

## Reflection

The capstone brief asks for this explicitly, as part of the final presentation: "a candid reflection on what you learned and what you would do differently." Most of that honesty is already scattered through the sections above, as it happened — collected here in one place.

**What actually turned out to matter.** Reasoning beat optimization, and it wasn't close — switching the extraction signature to `ChainOfThought` closed nearly the entire gap to a strong extractor; `BootstrapFewShot` and GEPA on top of that added deltas smaller than their own noise floors, and on the fresh AMZN/AAPL holdout, optimization actually made things worse. Testing for overfitting is not optional busywork — the clean 0/33 specificity result completely hid a real, fixable segment-misattribution bug that only the fresh-holdout test surfaced; a result measured only against the companies it was tuned on isn't evidence of anything beyond those companies. A near-zero-false-positive checker is worth more than a stronger extractor — nearly every real bug found across this project was upstream of the Reconciler, never in its own arithmetic, and the maker/checker precision-ceiling math bears that out directly. Naming a constraint openly beats working around it silently — the GRPO group size (6, not the capstone brief's suggested 8) and the GPU (a single 24GB RTX 4090, not the 32GB card the brief and the original planning doc assumed) are both real deviations from plan, documented in `phase7/train_grpo.py`'s own comments rather than smoothed over. The DSPy compiled-artifact freeze was the opposite lesson, learned the hard way: an undocumented, invisible constraint that cost real time before anyone noticed it was there.

**What would be done differently, with more time.** Segment/dimensional XBRL coverage is the largest remaining gap — most claims resolve "unverifiable" because the company-facts API this project reads has no Axis/Member dimensional breakdown at all; a proper XBRL dimensional parser (the full instance document, not just company-facts) would likely unlock most of the currently-unresolvable segment claims (Azure, Data Center, XBOX revenue) instead of correctly-but-unsatisfyingly declining them. GAAP/non-GAAP disambiguation needs more than a dictionary — the CSCO and SNPS cases are a real starting training set for a small classifier or a second-pass check purpose-built for this one ambiguity. A frontier-model comparison point is still missing — every result in this project, including the naive-baseline ablation, was measured against exactly one self-hosted `gpt-oss-20b` endpoint, so "is structure necessary" and "is structure necessary for a model this size" remain conflated. And the GRPO group size was never actually A/B tested against the brief's suggested G=8 — access to a larger card would isolate whether group size was ever the real reason `bps_change` stayed stuck, rather than leaving that as an open, unconfirmed hypothesis.
