...
**Result: 18 real matches**, all from Discover Financial Services and Rithm Capital Corp. — the two companies whose real MD&A prose happened to narrate the exact figures Phase A found restated. Output: [`research/data/phase_b_matches.json`](../research/data/phase_b_matches.json).

**A real reliability bug, found the hard way.** The first full 60-company run stalled for 2h15m+ with no visible progress and, more seriously, no incremental saving — all matches lived only in memory, so the 18 already-found matches would have been lost entirely if killed. Root cause was two compounding issues: Python buffers stdout when piped (hiding `print()` progress until process exit), and matches were only written to disk at the very end. Fixed with `flush=True` on every print, a `save_matches()` call after every filing (not just at the end), and a hard per-chunk wall-clock timeout (`EXTRACTION_TIMEOUT_SECONDS=45`, via `ThreadPoolExecutor`) independent of whatever retry/timeout behavior the LLM client does internally. Re-run scoped to the two known-good companies (`TARGET_CIKS`) — completed in under a minute, reproducing all 18 matches cleanly. The full 60-company scan was not re-run at that scale; 18 matches from 2 companies is what's reported, not rounded up to the larger candidate pool.

## Phase C — the backtest itself

[`research/run_backtest.py`](../research/run_backtest.py). For each of the 18 matched claims, this builds a `Claim` directly from the fingerprint's own known `period_start`/`period_end` and the matched prose's `claimed_value`, then calls the actual `tools.reconciler.reconcile()` — the same function every other result in this project uses — twice:
...
|---|---|---|
| **expected flip** | 13 | consistent at filing → inconsistent once the restatement existed — the clean target pattern |
| **already inconsistent at filing** | 4 | the prose's own rounding missed tolerance before any restatement existed — a real catch, for an unrelated reason |
...
**17 of 18 (94%) would have been flagged inconsistent by the time each restatement existed, using only data that existed at each point in time.** 13 of those show the full target pattern — the claim looked fine on filing day and only became detectably wrong once the SEC amendment landed, exactly what the bitemporal `as_of` design exists to catch.

Two examples of the clean flip, from [`research/data/backtest_results.json`](../research/data/backtest_results.json):
...
18 matches come from 2 companies out of the 541 with real restatement fingerprints. The number to trust is 13,827 fingerprints / 541 companies — counted directly from real SEC data — not the smaller 18 pushed all the way through prose-matching and backtesting, which reflects Phase B's scoped rerun after the reliability fix, not a re-run of the full 60-company candidate pool. No LLM extraction runs on prose that hasn't been checked; no reconciliation runs on a claim this pipeline didn't actually extract and match.

## Reproducing

```bash
uv run python -m research.find_restatements     # ~15 min, hits EDGAR full-text search + company-facts API
uv run python -m research.match_prose_claims     # scoped to TARGET_CIKS by default; real LLM extraction calls
